...
import { motion, AnimatePresence } from "framer-motion";
import { IoIosAdd, IoIosRemove } from "react-icons/io";
import { useLazyLoad } from "@/hooks/useLazyLoad";  // Import the custom lazy load hook
...
export const FAQs = () => {
  const [openAccordion, setOpenAccordion] = useState<number | null>(null);
  const [faqsData, setFaqsData] = useState<any[]>([]);  // Store the fetched FAQs data
  const [hasFetched, setHasFetched] = useState(false);  // Check if data is already fetched

  const t = useTranslations("home");
  const locale = useLocale();
  const { fetchLocalizedTable } = useSupabase();

  const { ref, isVisible } = useLazyLoad({ triggerOnce: true });  // Use lazy loading hook
  ...
  return (
    <section ref={ref} className="flex w-full flex-col items-center py-[4%] bg-[#F4F5F6] mt-20">
      <h1 className={`${styles.sectionHeadText} text-[#0D0D28]`}>
        {t("faq_title")}
      </h1>
      <h3 className="text-base text-[#606877] text-center max-w-xl mt-2 leading-relaxed">
      ...
      <div className="flex flex-col w-full max-w-2xl gap-4 mt-10 px-4 sm:px-0">
        {/* Render FAQs only when data is loaded and visible */}
        {faqsData &&
          faqsData.map(
            (faq: { id: number; question: string; answer: string }, index: number) => (
              <AccordionItem
                key={faq.id}
                question={faq.question}
                answer={faq.answer}
