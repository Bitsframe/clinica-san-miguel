...
export const Map: React.FC<MapProps> = ({ height, location, title }) => {
  ...
  return (
    ...
    >
      {(!isVisible || !isLoaded) && (
        <MapLoadingState label={t("loading_map")} />
      )}
...
export const GroupedMap = ({
  ...
  const src = location
    ? `https://www.google.com/maps/d/embed?mid=${location}`
    : "";
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !containerRef.current || !src) return;
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-full w-full overflow-hidden bg-[#F4F5F6]"
      style={{ minHeight: height }}
    >
      {(!isVisible || !isLoaded) && (
        <MapLoadingState label={t("loading_map")} />
